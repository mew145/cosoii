...
CREATE TABLE usuarios (
    ...
    -- Campos para OAuth
    provider_oauth VARCHAR(50), -- 'google', 'linkedin', 'github', null para email/password
    provider_id VARCHAR(255), -- ID del usuario en el proveedor OAuth
...
-- Tabla: tipos_activo_informacion
...
-- Tabla: clasificaciones_seguridad
...
-- Tabla: dominios_iso27001
CREATE TABLE dominios_iso27001 (
    id_dominio SERIAL PRIMARY KEY,
    codigo_dominio VARCHAR(10) NOT NULL, -- A.5, A.6, etc.
...
CREATE TABLE controles_iso27001 (
    ...
    codigo_control VARCHAR(20) NOT NULL, -- A.5.1, A.6.1, etc.
    nombre_control VARCHAR(300) NOT NULL,
    descripcion_control TEXT,
    objetivo_control TEXT,
    tipo_control VARCHAR(20) NOT NULL, -- 'preventivo', 'detectivo', 'correctivo'
    categoria_control VARCHAR(20) NOT NULL, -- 'tecnico', 'administrativo', 'fisico'
...
CREATE TABLE evaluaciones_riesgo_iso (
    ...
    tratamiento_riesgo VARCHAR(20) NOT NULL, -- 'aceptar', 'mitigar', 'transferir', 'evitar'
...
-- Tabla: controles_aplicables_evaluacion
...
-- Tabla: tipos_incidente_seguridad
...
CREATE TABLE incidentes_seguridad (
    ...
    severidad_incidente VARCHAR(20) NOT NULL, -- 'baja', 'media', 'alta', 'critica'
...
-- Tabla: activos_afectados_incidente
CREATE TABLE activos_afectados_incidente (
    ...
    id_activo INTEGER REFERENCES activos_informacion(id_activo),
    nivel_afectacion VARCHAR(20), -- 'bajo', 'medio', 'alto'
    descripcion_afectacion TEXT
);

-- Tabla: mapeo_coso_iso
...
-- Insertar usuario superadmin inicial (debes cambiar el password después)
...
-- Insertar algunos controles ISO 27001 principales (se pueden agregar los 93 completos después)
...
CREATE INDEX idx_auditorias_riesgo ON auditorias(id_riesgo);
CREATE INDEX idx_auditorias_auditor ON auditorias(id_auditor);
CREATE INDEX idx_auditorias_estado ON auditorias(id_estado_auditoria);

-- Índices para bitácora
CREATE INDEX idx_bitacora_usuario ON bitacora(id_usuario);
CREATE INDEX idx_bitacora_fecha ON bitacora(fecha_accion_bitacora);
CREATE INDEX idx_bitacora_modulo ON bitacora(modulo_afectado);
...
CREATE INDEX idx_evaluaciones_iso_evaluador ON evaluaciones_riesgo_iso(id_evaluador);
CREATE INDEX idx_incidentes_tipo ON incidentes_seguridad(id_tipo_incidente);
CREATE INDEX idx_incidentes_asignado ON incidentes_seguridad(id_asignado_a);
CREATE INDEX idx_mapeo_riesgo ON mapeo_coso_iso(id_riesgo);
CREATE INDEX idx_mapeo_evaluacion_iso ON mapeo_coso_iso(id_evaluacion_iso);

-- =============================================
-- ROW LEVEL SECURITY (RLS) - OPCIONAL
-- =============================================

-- Habilitar RLS en las tablas principales (opcional según tu necesidad de seguridad)
...
-- Políticas RLS ejemplo (ajusta según tus necesidades)
CREATE POLICY "Usuarios pueden ver solo su perfil" ON usuarios
FOR SELECT USING (auth.uid() = id_usuario);

CREATE POLICY "Gerentes pueden ver riesgos de sus proyectos" ON riesgos
FOR SELECT USING (EXISTS (
    SELECT 1 FROM proyectos p 
    WHERE p.id_proyecto = riesgos.id_proyecto 
    AND p.id_gerente_proyecto = auth.uid()
));

CREATE POLICY "Propietarios pueden ver sus activos" ON activos_informacion
FOR SELECT USING (id_propietario = auth.uid() OR id_custodio = auth.uid());

CREATE POLICY "Responsables pueden ver sus controles ISO" ON controles_iso27001
FOR SELECT USING (id_responsable = auth.uid());

-- =============================================
-- TRIGGERS PARA AUDITORÍA AUTOMÁTICA
-- =============================================

-- Función para registrar cambios en bitácora
CREATE OR REPLACE FUNCTION registrar_cambio_bitacora()
RETURNS TRIGGER AS $$
BEGIN
...
CREATE TRIGGER trigger_bitacora_usuarios
    AFTER INSERT OR UPDATE OR DELETE ON usuarios
    FOR EACH ROW EXECUTE FUNCTION registrar_cambio_bitacora();

CREATE TRIGGER trigger_bitacora_riesgos
    AFTER INSERT OR UPDATE OR DELETE ON riesgos
    FOR EACH ROW EXECUTE FUNCTION registrar_cambio_bitacora();

CREATE TRIGGER trigger_bitacora_activos
    AFTER INSERT OR UPDATE OR DELETE ON activos_informacion
    FOR EACH ROW EXECUTE FUNCTION registrar_cambio_bitacora();

CREATE TRIGGER trigger_bitacora_controles_iso
    AFTER INSERT OR UPDATE OR DELETE ON controles_iso27001
    FOR EACH ROW EXECUTE FUNCTION registrar_cambio_bitacora();

CREATE TRIGGER trigger_bitacora_incidentes
    AFTER INSERT OR UPDATE OR DELETE ON incidentes_seguridad
    FOR EACH ROW EXECUTE FUNCTION registrar_cambio_bitacora();
...
SELECT 
    ...
    (SELECT COUNT(*) FROM evaluaciones_riesgo_iso WHERE nivel_riesgo_aceptable = false) as riesgos_no_aceptables;

-- Vista: Mapeo COSO-ISO integrado
CREATE VIEW vista_mapeo_coso_iso AS
SELECT 
    r.titulo_riesgo,
    cr.nombre_categoria_riesgo,
    r.nivel_riesgo_calculado,
    ai.nombre_activo,
    cs.nombre_clasificacion,
    eri.riesgo_inherente,
    eri.riesgo_residual,
    eri.tratamiento_riesgo,
    ci.nombre_control as control_coso,
    ciso.nombre_control as control_iso,
    m.tipo_relacion,
    m.descripcion_relacion
FROM mapeo_coso_iso m
JOIN riesgos r ON m.id_riesgo = r.id_riesgo
JOIN categorias_riesgo cr ON r.id_categoria_riesgo = cr.id_categoria_riesgo
LEFT JOIN evaluaciones_riesgo_iso eri ON m.id_evaluacion_iso = eri.id_evaluacion_iso
LEFT JOIN activos_informacion ai ON eri.id_activo = ai.id_activo
LEFT JOIN clasificaciones_seguridad cs ON ai.id_clasificacion = cs.id_clasificacion
LEFT JOIN controles_internos ci ON m.id_control_interno = ci.id_control
LEFT JOIN controles_iso27001 ciso ON m.id_control_iso = ciso.id_control_iso;

-- =============================================
-- COMENTARIOS FINALES
-- =============================================

-- Este esquema integra completamente COSO ERM 2017 con ISO 27001:2022
-- Incluye:
-- 1. Todas las tablas originales de COSO II
-- 2. Tablas específicas para ISO 27001 (activos, controles, evaluaciones, incidentes)
-- 3. Tablas de mapeo para integración entre ambos marcos
-- 4. Soporte para autenticación OAuth
-- 5. Auditoría automática con triggers
-- 6. Vistas para reportes y dashboards
...
-- 2. Configurar las políticas RLS según tus necesidades específicas
-- 3. Completar los 93 controles ISO 27001 si es necesario
-- 4. Ajustar los datos iniciales según tu organización